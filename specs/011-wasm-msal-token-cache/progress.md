...
- **A failed silent refresh no longer reports success** (security HIGH; carried over from
  `specs/012` as "latent"). It is not latent: security's reframing is that with a 24h non-sliding
...
- Why not now: it changes public behavior of a published package (`LoginAsync(ct)` starts
  ...
  `specs/012`). Worth doing properly, not as a drive-by.
...
3. ~~The empty-token refresh finding (`specs/012-msal-auth-fixes/progress.md`) is still open and
   still needs a deterministic repro before it is fixed.~~ **Fixed 2026-08-23** — see the second
   review panel section and `specs/012-msal-auth-fixes/progress.md`.
...
`specs/012-msal-auth-fixes/progress.md`. The loop is a strictly smaller behavior than
...
the same pass (`specs/lessons.md`, this file, `spec.md`, `specs/012`, `specs/010`). Fixed:
...
- **Empty-token / silent-refresh fix** (the security HIGH carried from `specs/012`, finally with a
