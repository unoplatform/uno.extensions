...
- The commit messages and problem statement verbatim — all seven agents benefit from understanding intent: `quality` for requirement alignment, `contract` to distinguish intentional vs accidental API changes, `operability` and `performance` to know which targets are in scope, `skeptic` to verify the implementation matches the stated goal.
