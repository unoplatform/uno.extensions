...
Uno Platform 7.0 renders with Skia everywhere, so `Uno.WinUI` ships library assets only for `net10.0` and `net10.0-windows10.0.19041.0`. The published libraries target those two plus `net10.0-android` and `net10.0-ios`: those build against `Uno.WinUI`'s `net10.0` asset, and Uno 7 deploys a library's own Android/iOS build on device, so `__ANDROID__`/`__IOS__` code runs. There is no `browserwasm` or `desktop` library build — pick those paths at runtime (`OperatingSystem.IsBrowser()`, `PlatformHelper.IsWebAssembly`). App heads add `net10.0-desktop` and `net10.0-browserwasm`. Mac Catalyst is not supported on 7.0. The Uno SDK version is pinned in `global.json` (`Uno.Sdk.Private`).
