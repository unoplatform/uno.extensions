...
The **View** is the UI. You can write it with XAML, C#, or a combination of the two, much as you would when using another design pattern. You use data binding to bind to the ViewModel to separate the View from ViewModel. MVUX does use a ViewModel, but in a different way than the MVVM pattern. For now let's refer to the ViewModel as just Model.

### Update

When the user interacts with the View and provides input or action, it will trigger an **Update** that changes the Model.

In the standard MVU pattern, an updated Model will create a new View for the Model.  
In MVUX the existing View will be updated via data-binding by a ViewModel generated by MVUX, the ViewModel wraps around the Model class, and tracks changes to the Model.
...
The **Model** that the Update creates (and is wrapped in a generated ViewModel) is typically a simple class or record that contains only data and no logic. For this reason, a record is usually the most appropriate data structure for the Model. C# records are immutable (read-only), and their use is encouraged by MVUX. It will be explained further later.
