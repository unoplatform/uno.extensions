...
When registering routes, we can take advantage of the `ViewMap` object and its variations `DataViewMap` and `ResultDataViewMap`, to correlates Views with ViewModels, specify the type of parameters ViewModels may take, and specify the type of return coming from a navigation.
...
The **Nested** property allows you to define hierarchical routes. It creates a parent-child relationship between routes, enabling scoped navigation within sections of your app. This helps organize navigation structure and manage complex navigation scenarios more effectively. In the following example `MainPage` has two nested routes "ForYouTab" and "FavoritesTab", which means that those views will be displayed within `MainPage`:
...
**Init** enables you to override the navigation request, allowing for custom initialization actions before displaying the associated view.
