...
Regions are particularly useful with navigational controls, such as `NavigationView` and `TabBar` for showing or hiding, and opening or closing content and pages. In these cases, it's recommendable to have a `Grid` element as the parent of the navigational control, that we will call as "Parent Grid" in this guide. Additionally, it is essential to define a `Grid` that will hold the content to be displayed based on the selected `NavigationViewItem` or `TabBarItem`, and we will call it a "Content Grid".
...
The next important thing to do is to set the region names to associate the navigational control items to the content to be displayed. But before we give an example, we need to consider that there are two ways of setting the content in the Content `Grid`:
...
- 1. Set the content directly inside the Content `Grid`. Then we can map each content with a region name using `Region.Name`:
   ...
   For this case we can then associate the navigational control items with the contents by adding same region names:
