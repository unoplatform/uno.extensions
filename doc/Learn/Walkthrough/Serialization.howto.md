...
protected override void OnLaunched(LaunchActivatedEventArgs args)
{
    var builder = this.CreateBuilder(args)
        .Configure(host =>
        {
            host.UseJsonSerializationResolvers(PersonContext.Default);
        });
}
```

`UseJsonSerializationResolvers` registers the compiled metadata so the serializer can avoid runtime reflection.
...
```csharp
host.UseJsonSerializationResolvers((context, services) =>
{
