...
- **Cancelled or failed sign-in** — the user backing out of the browser surface, a timeout, an identity-provider error — returns `false` from `LoginAsync` and leaves any previously cached session untouched. Only a `CancellationToken` you cancel yourself surfaces as `OperationCanceledException`.
- **Silent refresh** redeems the stored refresh token against the token endpoint. A refresh the identity provider *rejects* (`invalid_grant` and any other OAuth error code, including provider-specific ones such as `interaction_required`; `temporarily_unavailable` and `server_error` excepted) returns `false` and signs the user out rather than reporting a stale success. A refresh that never reached a verdict — the endpoint unreachable, a server error — keeps the current tokens and logs a `Warning`, so an offline start does not sign the user out.
