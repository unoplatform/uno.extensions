...
> [!IMPORTANT]
> > You should never store any credentials in appsettings or code, such as the sample content shown above for the Access Token.  
> For development purposes, it is better to use `dotnet user-secrets` via the IDE Terminal.  
> If you are publishing your app, services like [Azure Key Vault](https://learn.microsoft.com/en-us/azure/devops/pipelines/process/set-secret-variables?view=azure-devops&tabs=yaml%2Cbash)  are a good option for safe handling of sensitive data.
> Azure Key Vault can also be easily connected via the available [Options for CI/CD in the **Uno App Templates**](https://platform.uno/docs/articles/getting-started/wizard/using-wizard.html#11-ci-pipeline).
