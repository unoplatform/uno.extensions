...
> [!IMPORTANT]
> You should never clear text any credentials in appsettings or code, like the sample content shown above of the Access Token. For Development purposes, you can instead and better use `dotnet user-secrets` via the IDE Terminal for example or and if you are publishing your App, Services like [Azure Key Vault](https://learn.microsoft.com/en-us/azure/devops/pipelines/process/set-secret-variables?view=azure-devops&tabs=yaml%2Cbash) are a good Option to choose for safe handling of vulnerable Data like these and can also easily connected via the available [Options for CI/CD in the **Uno App Templates**](https://platform.uno/docs/articles/getting-started/wizard/using-wizard.html#11-ci-pipeline).
