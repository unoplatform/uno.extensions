...
- **Bind the response to the request.** The listener completes on the first request to the callback path, and on a desktop that path is reachable by any page open in the same browser or any local process — the broker cannot know which request it started. The OIDC provider binds every response with `state` and PKCE itself; the Web provider is protocol-agnostic, so put the literal `{State}` token in `LoginStartUri` (`&state={State}`) and the provider generates a fresh value per sign-in and rejects any response that does not echo it. Without it, a page the user happens to be visiting during sign-in could hand the app a token of its choosing — so a sign-in on a loopback callback whose `LoginStartUri` has no `{State}` logs a `Warning`. A response that fails the check is rejected without touching the session the user already has.
- If no redirect arrives within `WinRTFeatureConfiguration.WebAuthenticationBroker.AuthenticationTimeout`, the flow is reported as cancelled (WinRT has no timeout status) with `ResponseErrorDetail` set to `DesktopWebAuthenticationBrokerProvider.TimeoutErrorDetail`; the Web and OIDC providers report that as a sign-in that did not happen (`LoginAsync` returns `false`, the previous session is kept) and log it as a timeout, and a `Warning` names the callback that never fired.
- One flow runs at a time. Starting a second sign-in or sign-out while one is still waiting for its redirect throws `InvalidOperationException` rather than failing to bind the port — disable the sign-in button while `LoginAsync` is pending.
- A `response_mode=form_post` body larger than 32 KB is rejected and the listener keeps waiting for the real response.
- To open the sign-in page some other way (a specific browser, a private window), derive from `DesktopWebAuthenticationBrokerProvider`, override `LaunchBrowserAsync`, and register your type with `ApiExtensibility` before `AddWeb`/`AddOidc` run — registration is first-wins. The log line *"WebAuthenticationBroker flows are handled by the desktop loopback broker"* on the first flow tells you the built-in one is active.
