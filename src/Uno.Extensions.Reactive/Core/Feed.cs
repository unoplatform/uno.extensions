...
public static partial class Feed
{
	...
	public static IFeed<T> Async<T>(AsyncFunc<T?> valueProvider, Signal? refresh = null)
		where T : struct
		=> refresh is null
			? AttachedProperty.GetOrCreate(valueProvider, static vp => new AsyncFeed<T>(vp.SomeOrNone(), rawDataProvider: vp))
			: AttachedProperty.GetOrCreate(refresh, valueProvider, static (r, vp) => new AsyncFeed<T>(vp.SomeOrNone(), r, vp));
