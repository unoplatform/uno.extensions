...
internal static class MobileRuntimeTestsAutostart
{
	...
	public static void StartIfRequested(Window window)
	{
		...
		// A continuation, not a catch-all around the run: it observes every outcome of the task -
		// fault, cancellation, success - and the process exits with the matching code whatever
		// happened, without a generic catch clause to argue about.
		_ = Task.Run(() => RunAsync(window, resultFile))
			.ContinueWith(run => Exit(run, resultFile), TaskScheduler.Default);
	}

	private static void Exit(Task run, string resultFile)
	{
		...
		try
		{
			var failure = run.Exception?.GetBaseException() ?? (object)run.Status;
			Console.Error.WriteLine($"MobileRuntimeTestsAutostart failed: {failure}");
		}
		finally
		{
			Environment.Exit(1);
	...
	private static Task<string> RunTestsAsync(
		...
		=> OnDispatcherAsync(
			...
			async () =>
			{
				...
				return unitTests.NUnitTestResultsDocument;
			},
			"runtime-tests");
	...
	private static Task<UnitTestsControl?> FindOnUIThreadAsync(DispatcherQueue dispatcher, Window window)
		=> OnDispatcherAsync(
			dispatcher,
			() => Task.FromResult(window.Content is DependencyObject root ? FindFirstOfType<UnitTestsControl>(root) : null),
			"find");

	/// <summary>
	/// Runs <paramref name="callback"/> on the dispatcher and hands its result - or whatever it threw -
	/// back to the awaiting worker thread.
	/// </summary>
	/// <remarks>
	/// Deliberately not <c>DispatcherQueueExtensions.ExecuteAsync</c> from Uno.Extensions.Core.UI: that
	/// ignores a rejected <see cref="DispatcherQueue.TryEnqueue(DispatcherQueueHandler)"/> (the awaiter then hangs) and rethrows
	/// into the dispatcher after forwarding, which crashes the app before <see cref="Exit"/> can log the
	/// cause.
	/// </remarks>
	private static Task<T> OnDispatcherAsync<T>(DispatcherQueue dispatcher, Func<Task<T>> callback, string what)
	{
		...
		if (!dispatcher.TryEnqueue(async () =>
		{
			try
			{
				tcs.TrySetResult(await callback().ConfigureAwait(true));
			}
			catch (Exception ex)
			{
				// Marshalling, not handling: everything the callback threw belongs to the awaiter, the
				// same way Task.Run forwards it. Anything narrower would leave the awaiter hanging.
		...
		}))
		{
			...
			throw new InvalidOperationException(
				$"DispatcherQueue rejected the {what} work item - dispatcher is shutting down.");
		}
