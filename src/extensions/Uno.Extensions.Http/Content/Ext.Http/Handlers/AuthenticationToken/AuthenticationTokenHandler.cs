...
namespace Uno.Extensions.Http.Handlers
{
        ...
        where TAuthenticationToken : IAuthenticationToken
    {
        ...
        public virtual bool ShouldIncludeToken(HttpRequestMessage request)
            => (request is not null) && request.Headers.Authorization != null;
        ...
        public virtual bool IsUnauthorized(HttpRequestMessage request, HttpResponseMessage response)
            => (response is not null) && response.StatusCode == HttpStatusCode.Unauthorized;
        ...
        protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            ...
            var token = await _tokenProvider.GetToken(cancellationToken, request);

            var response = await SendWithAuthenticationToken(cancellationToken, request, token);
            ...
            if (!token.CanBeRefreshed)
            {
                ...
                await _tokenProvider.NotifySessionExpired(cancellationToken, request, token);

                return response;
            }

            var refreshedToken = await RefreshAuthenticationToken(cancellationToken, request, token);
            ...
            response = await SendWithAuthenticationToken(cancellationToken, request, refreshedToken);
            ...
            if (IsUnauthorized(request, response))
            {
                ...
                await _tokenProvider.NotifySessionExpired(cancellationToken, request, refreshedToken);
        ...
        /// <returns>Return auth token <typeparamref name="TAuthenticationToken"/>.</returns>
