...
public class SystemTextJsonSerializer : ISerializer
{
	...
	public SystemTextJsonSerializer(IServiceProvider services, JsonSerializerOptions? serializerOptions = null)
	{
		...
		// Need to use `.GetService<…>()` in order for `.ConfigureSerializationOptions()` callbacks to be invoked.
		_serializerOptions = services.GetService<IOptions<SerializationOptions>>()?.Value?.SerializerOptions ??
			serializerOptions ??
			SerializationOptions.DefaultSerializerOptions;
	}
	...
	private static Exception CreateInvalidOperationException(Type targetType)
		=> new InvalidOperationException("Reflection-based serialization has been disabled for this application. " +
			"Use the IServiceCollection.AddJsonTypeInfoResolvers() or IHostBuilder.UseSerializationResolvers() extension methods to enable JSON deserialization " +
