...
namespace Umbrella.Presentation.Feeds.Collections._BindableCollection.Data
{
	...
	///		2. Buffer the changes that occures on collection (on Context thread)
	///		3. Replay all the changes from delta (1.) and buffer (2.) (on Context thread)
	///		4. Forward future changes that occures on collection (on Context thread) to the target CollectionChangedFacet
	/// </summary>
	internal class DataLayerChangesBuffer : IDisposable
	{
		...
		/// <summary>
		/// Gets the source collection handle by this changes manager
		/// </summary>
		...
		public DataLayerChangesBuffer UpdateTo(IObservableCollection source)  
			=> new DataLayerChangesBuffer(this,  _layer, source, _changesTracker);

		public DataLayerInitializer Initialize(ILayerTracker tracker)
		{
			...
			return new DataLayerInitializer(oldItems, changes, newItems, _layer.Items, tracker.Context, this);
		}
		...
		public void Start()
		{
			...
			{
				throw new InvalidOperationException($"The buffer is not in the expected state (expected: 'Initalized', actual: {_state})");
			}
		...
		public IObservableCollectionSnapshot Stop()
		{
			...
			if (Interlocked.CompareExchange(ref _state, State.Stopped, State.Initialized) == State.Initialized
				|| Interlocked.CompareExchange(ref _state, State.Stopped, State.Running) == State.Running)
			{
		...
		private void Buffer(RichNotifyCollectionChangedEventArgs arg)
		{
			...
			// Wait for initialization completion before self unbuffering
